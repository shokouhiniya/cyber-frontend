...
export function PostsList({ data, loading, total, filters, onSearchOpen, onClearSearch }) {
  ...
  const displayedPosts = expanded ? filteredData : filteredData.slice(0, 10);
  ...
  return (
    <Card sx={{ borderRadius: 2.5, overflow: 'hidden', boxShadow: theme.shadows[2] }}>
      {/* Header */}
      <Box sx={{ p: 2.5, background: `linear-gradient(135deg, ${alpha(theme.palette.success.main, 0.08)} 0%, ${alpha(theme.palette.warning.main, 0.08)} 100%)` }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Stack direction="row" alignItems="center" spacing={1.5}>
            ...
            <Box>
              <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>پست‌ها</Typography>
          ...
          <Stack direction="row" alignItems="center" spacing={1}>
            ...
            >
              <Iconify icon="solar:magnifer-bold-duotone" width={20} sx={{ color: hasActiveSearch ? 'info.main' : 'text.secondary' }} />
            </IconButton>
            ...
            <IconButton
              ...
              sx={{
                bgcolor: activeFiltersCount > 0 ? alpha(theme.palette.primary.main, 0.16) : alpha(theme.palette.grey[500], 0.08),
                position: 'relative',
              }}
            >
              <Iconify icon="solar:filter-bold-duotone" width={20} sx={{ color: activeFiltersCount > 0 ? 'primary.main' : 'text.secondary' }} />
            ...
            <IconButton
              ...
              sx={{ bgcolor: alpha(theme.palette.grey[500], 0.08), transition: 'transform 0.3s', transform: expanded ? 'rotate(180deg)' : 'none' }}
            >
      ...
      <Box sx={{ p: 2.5, pt: 2 }}>
        ...
        {!expanded && filteredData.length > 10 && (
          <Box sx={{ mt: 2, pt: 2, borderTop: `1px dashed ${alpha(theme.palette.grey[500], 0.2)}`, textAlign: 'center' }}>
            <Button variant="text" size="small" onClick={() => setExpanded(true)} startIcon={<Iconify icon="solar:alt-arrow-down-linear" width={16} />} sx={{ fontSize: 11, fontWeight: 600 }}>
              {filteredData.length - 10} پست دیگر
            </Button>
          </Box>
        )}
