...
// Round to the hour so the query key stays stable within the same hour
...
export function MostViewedPosts() {
  ...
  const { data: posts = [], isLoading } = useTopPosts(20, timeFilter);
    ...
    <>
      <Card sx={{ borderRadius: 2.5, overflow: 'hidden', boxShadow: theme.shadows[2] }}>
        <ButtonBase component="div" onClick={() => setSectionOpen((prev) => !prev)}
          sx={{ width: '100%', p: 2.5, display: 'block', textAlign: 'start', background: `linear-gradient(135deg, ${alpha(theme.palette.primary.main, 0.08)} 0%, ${alpha(theme.palette.info.main, 0.06)} 100%)` }}
        >
          <Stack direction="row" alignItems="center" justifyContent="space-between">
            <Stack direction="row" alignItems="center" spacing={1.5}>
              <Box sx={{ width: 40, height: 40, borderRadius: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: alpha(theme.palette.primary.main, 0.16) }}>
                <Iconify icon="solar:eye-bold-duotone" width={24} sx={{ color: theme.palette.primary.main }} />
              </Box>
              <Box>
                    ...
                    <PostCard
                      ...
                      badge={
                        <Stack direction="row" spacing={0.5}>
                          <Chip
                            icon={<Iconify icon="solar:eye-bold" width={10} />}
                            label={`${formatNum(post.viewCount)} بازدید`}
                            size="small"
                            sx={{ height: 20, fontSize: 8, fontWeight: 700, bgcolor: alpha(theme.palette.primary.main, 0.1), color: theme.palette.primary.main, '& .MuiChip-icon': { color: theme.palette.primary.main } }}
                          />
                        </Stack>
                      }
                    />
