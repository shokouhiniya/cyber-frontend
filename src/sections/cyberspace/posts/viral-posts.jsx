...
export function ViralPosts({ data, loading }) {
  ...
  // Score by repost + coverage potential (retweets weighted heavily, views as coverage)
  const scored = timeFiltered
    .filter((p) => (p.retweetCount || 0) > 0 || (p.viewCount || 0) > 0)
    .map((post) => {
      const reposts = post.retweetCount || 0;
      const coverage = post.viewCount || 0;
      const viralScore = reposts * 20 + coverage;
      return { ...post, reposts, coverage, viralScore };
    })
    ...
    <Card sx={{ borderRadius: 2.5, overflow: 'hidden', boxShadow: theme.shadows[2] }}>
      ...
      <ButtonBase
        ...
        sx={{
          ...
          background: `linear-gradient(135deg, ${alpha(theme.palette.info.main, 0.08)} 0%, ${alpha(theme.palette.warning.main, 0.08)} 100%)`,
        }}
      >
          ...
          <Stack direction="row" alignItems="center" spacing={1.5}>
            ...
            <Box>
              ...
              <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                پست‌هایی با بیشترین بازنشر و پوشش رسانه‌ای
              </Typography>
            </Box>
          </Stack>
          <Iconify
                ...
                {displayedPosts.map((post, index) => (
                  <Card
                    ...
                    sx={{
                      ...
                      borderRight: `4px solid ${alpha(theme.palette.info.main, 0.5)}`,
                      borderRadius: 2,
                  ...
                  >
                    <Stack spacing={1.5}>
                      {/* Header row */}
                      <Stack direction="row" alignItems="center" spacing={1.5}>
                        <Avatar sx={{ width: 36, height: 36, bgcolor: alpha(theme.palette.info.main, 0.16), color: theme.palette.info.main, fontSize: 13, fontWeight: 700 }}>
                        ...
                        <Typography variant="caption" sx={{ fontWeight: 800, fontSize: 10, color: theme.palette.info.main }}>
                          #{index + 1}
                        </Typography>
                      </Stack>
                      ...
                      {/* Two metrics: reposts + coverage */}
                      <Stack direction="row" spacing={3}>
                        <Stack direction="row" alignItems="center" spacing={0.5}>
                          <Chip
                            icon={<Iconify icon="solar:share-bold" width={12} />}
                            label={`${formatNum(post.reposts)} بازنشر`}
                            size="small"
                            sx={{
                        ...
                        <Stack direction="row" alignItems="center" spacing={0.5}>
                          <Chip
                            icon={<Iconify icon="solar:users-group-rounded-bold" width={12} />}
                            label={`${formatNum(post.coverage)} پوشش`}
                            size="small"
                            sx={{
                              height: 22, fontSize: 10, fontWeight: 700,
